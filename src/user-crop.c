...
G_DEFINE_TYPE (UserCropArea, user_crop_area, GTK_TYPE_DRAWING_AREA);

static inline guchar shift_color_byte (guchar b,
                                       int    shift)
{
    return CLAMP(b + shift, 0, 255);
}

static void shift_colors (GdkPixbuf *pixbuf,
                          gint       red,
                          gint       green,
                          gint       blue,
                          gint       alpha)
...
static void update_pixbufs (UserCropArea *area)
{
    gint width;
    gint height;
...
static void crop_to_widget (UserCropArea *area,GdkRectangle  *crop)
{
...
static gboolean user_crop_area_draw (GtkWidget *widget,cairo_t   *cr)
{
    GdkRectangle crop;
    gint width, height, ix, iy;
    UserCropArea *uarea = USER_CROP_AREA (widget);
...
static Range find_range (gint x,
                         gint min,
                         gint max)
{
    gint tolerance = 12;
...
static Location find_location (GdkRectangle *rect,
                               gint          x,
                               gint          y)
{
...
static void update_cursor (UserCropArea *area,
                           gint           x,
                           gint           y)
{
    gint cursor_type;
...
static gboolean user_crop_area_motion_notify_event (GtkWidget      *widget,
                                                    GdkEventMotion *event)
{
    UserCropArea *area = USER_CROP_AREA (widget);
    gint x, y;
...
static gboolean user_crop_area_button_press_event (GtkWidget      *widget,
...
static gboolean user_crop_area_button_release_event (GtkWidget      *widget,
                                                     GdkEventButton *event)
{
    UserCropArea *area = USER_CROP_AREA (widget);
    GdkRectangle crop;
...
static void user_crop_area_set_size_request (UserCropArea *area)
{
...
static void user_crop_area_finalize (GObject *object)
{
...
static void
user_crop_area_class_init (UserCropAreaClass *klass)
{
    GObjectClass   *object_class = G_OBJECT_CLASS (klass);
    GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

    object_class->finalize = user_crop_area_finalize;
    widget_class->draw = user_crop_area_draw;
    widget_class->button_press_event = user_crop_area_button_press_event;
    widget_class->button_release_event = user_crop_area_button_release_event;
    widget_class->motion_notify_event = user_crop_area_motion_notify_event;
}

static void
user_crop_area_init (UserCropArea *area)
{
...
{
    return g_object_new (USER_TYPE_CROP_AREA, NULL);
}
